...
| A4 | §5 Stack, §10, §12 LLM | **Superseded by A8.** Answer synthesis and knowledge extraction ran on a single third-party model API rather than Gemini via Vertex, at the product owner's direction. Embeddings stayed on Vertex (A3). | Recorded here because the module docstrings carried it and the amendments table did not. What replaced it, and why, is A8. |
| A5 | §15 API, §16, §21 S26–S28 | The handover surface ships as the **KT console**: a synchronous, on-demand, never-persisted executive summary and copilot (JSON responses) rather than `POST /v1/handover/{person_id} → 202 + job_id` and an SSE stream; the onboarding "ordered reading path" lands as the KT learning path computed from the recipient's visible claims and documents. | ADR 0016. Production deploys no worker service and no Redis, so an asynchronous job would enqueue rows nobody drains; the citation gate needs the whole answer before anything may be shown, so token streaming would stream ungated text. Streaming post-gate events and a background composer remain open decisions, not silent omissions. |
| A6 | §20 GCP deployment, §5 Queue | The production worker is a **private Cloud Run service rung by Cloud Tasks, at `min-instances=0`**, not the always-on `min-instances=1` worker §20 lists; there is no Redis in production and the reaper stays a scheduled job. A5's premise that "production deploys no worker service" no longer holds. | ADR 0017. Postgres is the queue and a push-rung service drains it within seconds of any enqueue, so an idle container would buy nothing but a standing bill; §20's own cost guardrails argue for zero. The nightly analysis agent (§13), when it lands, is a scheduled job for the same reason. |
| A8 | §5 Stack, §10, §12 LLM | Answer synthesis (`/v1/ask`, the KT copilot, the handover summary) **and** knowledge extraction run through **one ordered chain of three OpenAI-compatible vendors — Cerebras, OpenRouter, Groq** (`LLM_PROVIDER_ORDER`, `CEREBRAS_API_KEY` / `OPENROUTER_API_KEY` / `GROQ_API_KEY`, all three defaulting to the `gpt-oss-120b` family), not Gemini via Vertex and not the single vendor A4 named. Embeddings stay on Vertex (A3), unchanged. Extraction records the vendor and model that actually answered on every run and every claim. | ADR 0024. The single vendor A4 named returned HTTP 400 to 100% of 200 sampled production calls over thirty days: the KT copilot answered 503 and every nightly extraction recorded `provider_permanent`, with retrieval working perfectly throughout. One vendor's decision must not be able to make two surfaces unavailable with nothing able to route around it. The grounding gate, the quote gate and the model-agnostic frontend are unchanged by the provider — which is what made the swap a configuration-shaped change rather than a redesign. |
| A7 | §9 Ingestion, §20 GCP deployment | Connected providers are re-read on a **nightly schedule each organisation chooses in its own timezone** (default 01:00), driven by a Cloud Scheduler → Cloud Run job (`jutsu-sync`) that §20's table does not list. The organisations due are read from an **org-less schedule index** in a `sched` schema contained by privilege rather than row-level security, because no role in this deployment can enumerate `orgs` — including the migration role in production. | ADR 0018. §9 requires each stage to be a separately queued job and says nothing about what starts the first one, so until now nothing did: a connection nobody clicked was never re-read. ADR 0012's refusal of a cross-tenant sweeper stands — the clock reads an id, a timezone and an hour, and no tenant content at all. |
