...
| A4 | §5 Stack, §10, §12 LLM | Answer synthesis (`/v1/ask`, the KT copilot, the handover summary) and knowledge extraction run on the **Claude API through the official `anthropic` SDK** (`ANTHROPIC_API_KEY`, `JUTSU_ANSWER_MODEL` / `JUTSU_EXTRACTION_MODEL`, default `claude-opus-5`), not Gemini via Vertex. Embeddings stay on Vertex (A3). | The product owner provisioned the Claude key and directed its use; the module docstrings recorded it, the amendments table did not. The grounding gate, the quote gate and the model-agnostic frontend are unchanged by the provider. |
